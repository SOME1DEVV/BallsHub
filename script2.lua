...
local State = {E=false,We=false,Af=false,As=false,He=false,Hv=2,Rw=false}
local MenuKey = Enum.KeyCode.RightControl
local MenuKeyConn = nil
local Keybinds = {ESP=Enum.KeyCode.None,WeaponESP=Enum.KeyCode.None,AntiFlash=Enum.KeyCode.None,AntiSmoke=Enum.KeyCode.None,BigHead=Enum.KeyCode.None,RemoveWalls=Enum.KeyCode.None}
local PlayerToggles = {}
local PlayerList = {}
local Exiting = false
...
Characters.ChildAdded:Connect(function(model)
    ...
    if State.E and model:IsA("Model") and model.Name ~= LocalPlayer.Name then
        AddESP(model)
        UpdateESPColors()
        if State.We then
            UpdateWeapons()
...
Characters.ChildRemoved:Connect(function(model)
    if OriginalHeadSizes[model.Name] then
        OriginalHeadSizes[model.Name] = nil
    end
end)
